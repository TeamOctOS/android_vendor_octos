...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/octos-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
EOF
            ...
            case $1 in
                __oct_*) echo "For internal use only." ;;
                changes|for)
...
EOF
                    ;;
                *)
                    $FUNCNAME __oct_err_not_supported $1 && return
                    cat <<EOF
...
EOF
            ...
            ;;
        __oct_get_ref)
            $FUNCNAME __oct_err_no_arg $command $# && return 1
            ...
            ;;
        fetch|pull)
            $FUNCNAME __oct_err_no_arg $command $# help && return 1
            $FUNCNAME __oct_err_not_repo && return 1
            local change=$1
            shift
            git $command $@ http://$review/p/$project \
                $($FUNCNAME __oct_get_ref $change) || return 1
            ;;
        push)
            $FUNCNAME __oct_err_no_arg $command $# help && return 1
            $FUNCNAME __oct_err_not_repo && return 1
        ...
        __oct_err_not_repo)
            ...
            ;;
        __oct_err_not_supported)
            $FUNCNAME __oct_err_no_arg $command $# && return
            case $1 in
    ...
    #TODO: other special cases?
        *)
            $FUNCNAME __oct_err_not_supported $command && return 1
            $FUNCNAME __oct_err_no_arg $command $# help && return 1
            $FUNCNAME __oct_err_not_repo && return 1
            local args="$@"
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $OCTOS_FIXUP_COMMON_OUT ]; then
