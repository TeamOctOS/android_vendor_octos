...
function setup_vendor() {
    ...
    export OCTOS_ROOT="$3"
    if [ ! -d "$OCTOS_ROOT" ]; then
        echo "\$OCTOS_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$OCTOS_ROOT/$OUTDIR" ]; then
        mkdir -p "$OCTOS_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$OCTOS_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$OCTOS_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$OCTOS_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$OCTOS_ROOT"/vendor/octos/build/tools/smali/baksmali.jar
        export SMALIJAR="$OCTOS_ROOT"/vendor/octos/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$OCTOS_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$OCTOS_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$OCTOS_ROOT"/"$OUTDIR"/proprietary
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$OCTOS_ROOT"/"$OUTDIR"/radio
