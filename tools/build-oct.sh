#!/bin/bash
#
# octosify.sh: the overarching build script for the ROM.
# Copyright (C) 2016-2017 Team-OctOs Project
#
...
usage() {
    ...
    echo -e "  octosify.sh [options] device"
    echo ""
...
while getopts "c:d:gij:ko:rs:w:" opt; do
    case "$opt" in
    c) opt_clean="$OPTARG" ;;
    d) opt_ccache=1 ;;
    i) opt_ignore=1 ;;
...
if [ "$opt_clean" -eq 1 ]; then
    ...
    if [ -d "$OUTDIR/target" ]; then
        echo -e "${bldcya}Output directory is: ${bldylw}Clean${rst}"
        echo ""
else
        echo -e "${bldcya}Output directory is: ${bldgrn}Untouched${rst}"
        echo ""
    fi
fi
...
rm -f "$OUTDIR"/target/product/"$device"/oct_*-ota*.zip
