...
PRODUCT_VERSION_MILESTONE = M2

# Get the device name and set TO_DEVICE (strips to_ from TARGET_PRODUCT)
TO_DEVICE = $(shell echo -e $(TARGET_PRODUCT) | cut -d '_' -f2,3)
...
ifeq ($(TO_BUILDTYPE), RELEASE)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        TO_VERSION := OCT-O-$(PRODUCT_VERSION_MILESTONE)-$(shell date -u +%Y%m%d)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(TO_DEVICE)
#        ifeq ($(PRODUCT_VERSION_MAINTENANCE),0)
#            TO_VERSION := OCT-O-$(PRODUCT_VERSION_MILESTONE)-$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(TO_DEVICE)
#        else
#            TO_VERSION := OCT-O-$(PRODUCT_VERSION_MILESTONE)-$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(TO_DEVICE)
#        endif
    else
        ifeq ($(TARGET_BUILD_VARIANT),user)
            TO_VERSION := OCT-O-v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(TO_DEVICE)
        else
            TO_VERSION := OCT-O-v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(TO_DEVICE)
        endif
    endif
else
    ifeq ($(PRODUCT_VERSION_MINOR),0)
        TO_VERSION := OCT-O-$(TO_BUILDTYPE)$(TO_EXTRAVERSION)-$(shell date -u +%Y%m%d-%H%M)-$(TO_DEVICE)
    else
        TO_VERSION := OCT-O-$(TO_BUILDTYPE)$(TO_EXTRAVERSION)-$(shell date -u +%Y%m%d-%H%M)-$(TO_DEVICE)
    endif
endif
