...
ifeq ($(TO_BUILDTYPE), RELEASE)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        TO_VERSION := OCT-N-$(PRODUCT_VERSION_MILESTONE)-$(shell date -u +%Y%m%d)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(TO_DEVICE)
#        ifeq ($(PRODUCT_VERSION_MAINTENANCE),0)
#            TO_VERSION := OCT-N-$(PRODUCT_VERSION_MILESTONE)-$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(TO_DEVICE)
#        else
#            TO_VERSION := OCT-N-$(PRODUCT_VERSION_MILESTONE)-$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(TO_DEVICE)
#        endif
    else
        ifeq ($(TARGET_BUILD_VARIANT),user)
            TO_VERSION := OCT-N-v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(TO_DEVICE)
        else
            TO_VERSION := OCT-N-v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(TO_DEVICE)
        endif
    endif
else
    ifeq ($(PRODUCT_VERSION_MINOR),0)
        TO_VERSION := OCT-N-$(TO_BUILDTYPE)$(TO_EXTRAVERSION)-$(shell date -u +%Y%m%d-%H%M)-$(TO_DEVICE)
    else
        TO_VERSION := OCT-N-$(TO_BUILDTYPE)$(TO_EXTRAVERSION)-$(shell date -u +%Y%m%d-%H%M)-$(TO_DEVICE)
    endif
endif
